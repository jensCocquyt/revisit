...
**Built so far: the full pipeline.** The API serves `GET /health`, `POST /links` (validated, idempotent via the `Idempotency-Key` header, creating the link and exactly one enrichment job in a single transaction), `GET /links` (keyset-paginated, filterable by status and tag), `GET /links/:id` (with `tags` and `deadline` facets), `GET /links/:id/enrichment` (contract-validated, serving only evidence that resolves against stored page text), plus `/openapi.json` and Swagger UI at `/docs`. Cross-origin access is opt-in through `CORS_ORIGINS`. The worker claims jobs with `FOR UPDATE SKIP LOCKED` + lease, safely fetches and extracts pages, enriches through the `Enricher` seam (stub default, Bedrock opt-in), verifies evidence, and persists idempotently.
...
The `worker/enrichers/` package holds the seam and its implementations. `worker/enrichers/base.py` defines the seam: `Enricher` (an abstract base class; the owner prefers ABCs over `Protocol` for explicitness), the `EnrichmentInput`/`EnrichmentOutcome` dataclasses, and the `get_enricher` factory (all re-exported from `worker.enrichers`). Implementations live in their own modules inside the package: `worker/enrichers/stub.py` holds `StubEnricher`, which derives a deterministic contract-valid result from a SHA-256 of `(content, note, goal)` (same input, identical output, no network), returning pydantic model instances, not dicts. Model-backed enrichers share `worker/enrichers/model.py`: `ModelEnricher` owns the prompt, the contract schema with field guidance, the content cap, tag normalization, strict validation, timing, and error classification; a provider subclass implements only `_complete(system, user, schema)` and returns the JSON payload plus token counts. `worker/enrichers/bedrock.py` (`BedrockEnricher`, forced tool call via Converse) and `worker/enrichers/ollama.py` (`OllamaEnricher`, local `/api/chat` with `format` set to the schema, local testing only) are the two transports. Each provider's `prompt_version` is `<provider>-<PROMPT_VERSION>`; bump `PROMPT_VERSION` in `model.py` whenever the shared prompt changes. Selected by `ENRICHER` env var, default `stub`. Keep the stub as the default test path; new providers subclass `ModelEnricher` in their own module. Error classes for the failure taxonomy (`FetchTerminalError`, `FetchTransientError`, `EnricherError`) live together in `worker/errors.py`. `worker/smoke.py` (`python -m worker.smoke`) is the in-container contract smoke check CI runs.

The `worker` package deliberately mixes pipeline modules with runnable entry points/tools (`__main__`, `healthcheck`, `smoke`, `evals`). Don't move the tools out: `healthcheck` and `smoke` must run inside the runtime image (compose healthcheck, CI stack job), and `python -m worker.<tool>` requires living in the package. If the tool set grows beyond these, group them into a `worker/cli/` subpackage — still inside the package — rather than a folder outside it.
