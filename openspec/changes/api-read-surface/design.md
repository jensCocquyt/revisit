...
- *Why the row id rather than an encoded timestamp*: `created_at` has microsecond precision and JavaScript dates have milliseconds, so any timestamp that passes through the API process comes back truncated and would skip rows created in the same millisecond. Naming the cursor row instead keeps the comparison inside postgres, where both timestamps are compared at full precision. For the same reason the anchor is never read into JavaScript, not even to bind it back.
- *Why an existence check accompanies the subquery*: a correlated subquery yields `NULL` for a cursor that names no link, and a `NULL` row comparison drops every row, so an invented or cross-environment cursor would read as the empty last page. A preceding `EXISTS` query tells the two apart and rejects the unknown cursor with `400`, without materializing the timestamp.
