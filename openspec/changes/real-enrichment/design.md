...
- `toolConfig`: a single `record_enrichment` tool whose input schema is derived from the pydantic contract (`TypeAdapter(EnrichmentResult).json_schema()` — single source of truth), `toolChoice` forcing that tool.
- Response: extract the tool-use input, validate with the contract's JSON-mode `TypeAdapter`; any validation error, missing tool call, refusal, or SDK/API error raises `EnricherTransientError("invalid_model_output" | "enrich_error")`.
