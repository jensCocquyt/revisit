# link-reading Specification

## Purpose
Reading the saved library through the API: the keyset-paginated link listing and its status and tag filters, the endpoint serving a link's latest enrichment after contract validation with only the evidence that resolves against stored page text, the API-key coverage both read routes sit behind, and the opt-in cross-origin access a browser client needs.
## Requirements
